 ...
 * Prefer the explicitly-provided apiKey, otherwise fall back to Vite env var
 * `import.meta.env.VITE_GEMINI_API_KEY` when available.
 */
export function initialize(apiKey?: string) {
    const key = apiKey || (typeof (import.meta as any)?.env !== 'undefined' ? (import.meta as any).env.VITE_GEMINI_API_KEY : undefined);
    if (!key) {
